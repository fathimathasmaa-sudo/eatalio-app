...
export default function CustomerNavbar(){
 ...
 return <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-slate-200/80 bg-[#fafaf9]/98 shadow-[0_-4px_18px_rgba(15,23,42,0.06)] backdrop-blur" aria-label="Customer navigation">
  <div className="mx-auto flex h-16 max-w-3xl items-center justify-center px-4 sm:px-6">
   <div className="grid w-full max-w-xl grid-cols-4 items-center gap-1 rounded-2xl bg-white p-1.5 ring-1 ring-slate-200/80 shadow-sm">
    {items.map(([base,to,label,Icon])=><Link key={label} to={to} className={`relative flex items-center justify-center gap-1.5 rounded-xl px-3 py-2 text-xs font-medium transition ${active(base)?"bg-slate-950 text-white":"text-slate-600 hover:bg-slate-50 hover:text-slate-950"}`}><Icon size={16} strokeWidth={1.9}/><span>{label}</span>{label==="Cart"&&cartCount>0&&<span className={`flex min-w-4 items-center justify-center rounded-full px-1 text-[9px] font-bold ${active(base)?"bg-white text-slate-950":"bg-slate-950 text-white"}`}>{cartCount}</span>}</Link>)}
   </div>
  </div>
 </nav>;
}
