...
export default function KitchenView() {
  ...
  const start = async (o) => {
    if (statusFor(o) !== "Accepted") return;
    await updateDoc(doc(db, "orders", o.id), { orderStatus: "Preparing", status: "Preparing", updatedAt: serverTimestamp() });
  };
  const finish = async (o) => {
    if (statusFor(o) !== "Preparing") return;
    const next = o.orderType === "takeaway" ? "Ready for Pickup" : "Ready";
    await updateDoc(doc(db, "orders", o.id), { orderStatus: next, status: next, updatedAt: serverTimestamp() });
  };
