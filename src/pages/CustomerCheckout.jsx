...
export default function CustomerCheckout() {
  ...
  const currentType = types.find(([value]) => value === orderType) || types[1];
  const CurrentTypeIcon = currentType[1];
  const currentTypeLabel = currentType[2];
  ...
  return <div className="min-h-screen bg-[#fafaf9] pb-8 text-slate-950">
    ...
    <main className="mx-auto max-w-3xl space-y-4 px-5 py-5 sm:space-y-5 sm:px-8 sm:py-7">
      ...
      <div className="inline-flex items-center gap-2 rounded-full bg-white px-3.5 py-2 text-sm font-medium text-slate-600 ring-1 ring-slate-200 shadow-sm"><CurrentTypeIcon size={16} className="text-slate-500"/><span>{currentTypeLabel}</span>{orderType === "dine-in" && table && <><span className="text-slate-300">·</span><span>Table {table}</span></>}</div>
