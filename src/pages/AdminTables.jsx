...
export default function AdminTables() {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (u) => {
      ...
      try {
        const snapshot = await getDoc(doc(db, "staff", u.uid));
  ...
  useEffect(() => onSnapshot(
    ...
    (snapshot) => setTables(snapshot.docs.map((d) => ({ id: d.id, ...d.data() })).sort((a, b) => String(a.name || "").localeCompare(String(b.name || ""))))
  ), []);
  ...
  const sessionByTable = useMemo(() => {
    const map = {};
    orders.filter((o) => o.orderType === "dine-in" && o.tableId && o.tableSessionId).forEach((o) => {
      if (!map[o.tableId]) map[o.tableId] = { sessionId: o.tableSessionId, orders: [], total: 0, openOrders: 0 };
      if (map[o.tableId].sessionId !== o.tableSessionId) return;
      map[o.tableId].orders.push(o);
      map[o.tableId].total += Number(o.total || 0);
      if (!closedStatus.includes(o.orderStatus || (o.status === "Pending" ? "Received" : o.status))) map[o.tableId].openOrders += 1;
    });
    return map;
  }, [orders]);

  const url = (table) => `${window.location.origin}/?table=${encodeURIComponent(table.code || table.name)}&tableId=${encodeURIComponent(table.id)}`;
  const qrUrl = (table) => `https://api.qrserver.com/v1/create-qr-code/?size=900x900&margin=20&data=${encodeURIComponent(url(table))}`;
  ...
  const create = async (event) => {
    ...
    setBusy(true);
    try {
      const code = tableName.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
      ...
      alert(error.message || "Could not add table.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const remove = async (table) => {
    ...
    if (window.confirm(`Remove ${table.name}?`)) await deleteDoc(doc(db, "tables", table.id));
  };
  ...
  const print = () => {
    ...
    if (!windowRef) return;
    windowRef.document.write(`<html><body style="font-family:Arial;text-align:center;padding:60px"><h1>${selected.name}</h1><img style="width:520px" src="${qr}" alt="QR Code"/><p>Scan to order</p></body></html>`);
    ...
    <div className="min-h-screen bg-[#f7f7f5] text-slate-950">
      <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5">
          <div><h1 className="font-bold">Eatalio Admin</h1><p className="text-xs text-slate-400">Tables & QR codes</p></div>
      ...
      <main className="mx-auto max-w-6xl px-5 py-7">
        ...
        <form onSubmit={create} className="mt-6 flex max-w-xl gap-2">
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Table name or number, e.g. Table 12" className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none"/>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <div className={`mt-2 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold ring-1 ${open ? "bg-emerald-50 text-emerald-700 ring-emerald-100" : "bg-slate-50 text-slate-500 ring-slate-200"}`}>
                      <span className={`h-1.5 w-1.5 rounded-full ${open ? "bg-emerald-500" : "bg-slate-300"}`}></span>{open ? "Open" : "Available"}
                    </div>
                  </div>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/50 p-4">
          <div className="w-full max-w-md rounded-3xl bg-white p-6 text-center">
            ...
            {qr && <img src={qr} alt={`${selected.name} QR`} className="mx-auto my-6 w-72 rounded-xl"/>}
